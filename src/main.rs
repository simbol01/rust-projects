...
// functions to slice highest scores
...
fn save_highscores_and_lines(highscores: &[u32], number_of_lines: &[u32]) -> bool {
    ...
    write_into_file(
        format!("{}\n{}", s_highscores, s_number_of_lines).as_str(),
        HIGHSCORE_FILE,
    )
    .is_ok()
}

fn line_to_slices(line: &str) -> Vec<u32> {
    line.split(" ")
...
fn load_highscores_and_lines() -> Option<(Vec<u32>, Vec<u32>)> {
    if let Ok(content) = read_from_file(HIGHSCORE_FILE) {
        let mut line = content
            .splitn(2, "\n")
            .map(|line| line_to_slices(line))
            .collect::<Vec<_>>();

        if line.len() == 2 {
            let (line_number, highscore) = (line.pop().unwrap(), line.pop().unwrap());
            Some((highscore, line_number))
        } else {
            None
        }
    } else {
    ...
    if let Ok(mut square_texture) = texture_creator.create_texture_target(None, width, height) {
        canvas
            .with_texture_canvas(&mut square_texture, |texture| {
                ...
                //(r, g, b) => texture.set_draw_color(Color::RGB(r, g, b)),
                //_ => texture.set_draw_color(Color::RGB(0, 0, 0)),
                // }
...
// Struct to hold tetrimino
type Piece = Vec<Vec<u8>>;
type State = Vec<Piece>;
struct Tetrimino {
    states: State,
    x: isize,
    y: usize,
...
// traits for all to be generated terminos
trait TerminoGenerator {
    fn new() -> Tetrimino;
}

// Creating the various tetrimino stuctures
struct TetriminoI;
impl TerminoGenerator for TetriminoI {
    fn new() -> Tetrimino {
        Tetrimino {
            states: vec![
                vec![
...
struct TetriminoJ;
impl TerminoGenerator for TetriminoJ {
    fn new() -> Tetrimino {
        Tetrimino {
            states: vec![
                vec![
...
struct TetriminoL;
impl TerminoGenerator for TetriminoL {
    fn new() -> Tetrimino {
        Tetrimino {
            states: vec![
                vec![
...
struct TetriminoO;
impl TerminoGenerator for TetriminoO {
    fn new() -> Tetrimino {
        Tetrimino {
            states: vec![vec![
                vec![4, 4, 0, 0],
...
struct TetriminoS;
impl TerminoGenerator for TetriminoS {
    fn new() -> Tetrimino {
        Tetrimino {
            states: vec![
                vec![
...
struct TetriminoZ;
impl TerminoGenerator for TetriminoZ {
    fn new() -> Tetrimino {
        Tetrimino {
            states: vec![
                vec![
...
struct TetriminoT;
impl TerminoGenerator for TetriminoT {
    fn new() -> Tetrimino {
        Tetrimino {
            states: vec![
                vec![
...
// New type to hold the tetrimo infos
struct Tetris {
    game_map: Vec<Vec<u8>>,
    current_level: u32,
    score: u32,
...
fn is_time_over(tetris: &Tetris, timer: &SystemTime) -> bool {
    match timer.elapsed() {
        Ok(elapsed) => {
            let millis = elapsed.as_secs() as u32 * 1000 + elapsed.subsec_nanos() / 1_000_000_000;
...
fn main() {
    let ttf_context = sdl2::ttf::init().expect("SDL2 initialization failed");
    let sdl_context = sdl2::init().expect(
        "SDL initialization
failed",
    );
    let mut font = ttf_context.load_font("assets/hack.ttf", 128).expect(
        "
    ...
    loop {
        ...
        canvas
            .copy(
                ...
                None,
                Rect::new(
                    10, //(width - TETRIS_HEIGHT as u32 * 10) as i32 / 2 - 10,
                    (height - TETRIS_HEIGHT as u32 * 16) as i32 / 2 - 10,
                    TETRIS_HEIGHT as u32 * 10 + 20,
                    TETRIS_HEIGHT as u32 * 16 + 20,
                ),
            )
            .expect("Coudn't copy texture into windows");
        ...
        canvas
            .copy(
                ...
                None,
                Rect::new(
                    20, //(width - TETRIS_HEIGHT as u32 * 10) as i32 / 2,
        ...
        let rendered_text =
            create_texture_from_text(&texture_creator, &font, "test", 255, 255, 255)
                .expect("Cannot render text");
        canvas
            .copy(
                &rendered_text,
